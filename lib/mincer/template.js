...
function fetchCache(self, callback) {
  if (!cache) {
    callback();
    return;
  }

  cache.get(self.cacheKey, function (err, hash) {
    if (err && err.errno !== process.ENOENT) {
      callback(err);
      return;
    }

    if (hash && self.dataDigest === hash.dataDigest) {
...
function updateCache(self, data, callback) {
  var hash;
  ...
  hash = { dataDigest: self.dataDigest, data: data };
  cache.save(self.cacheKey, hash, function (err) {
    if (err) {
      callback(err);
      return;
    }
...
/**
 *  Template.cache -> String|Null
 *
 *  Cache db path. Disabled (`Null`) by default.
 **/
Object.defineProperty(Template, 'cache', {
  ...
  set: function (val) {
    ...
    cache = nStore.new(path.resolve(val));
  }
});
...
Template.prototype.cacheable = function (evaluator, callback) {
  ...
  fetchCache(self, function (err, data) {
    if (err || data) {
      callback(err, data);
      return;
    }

    evaluator.call(self, function (err, result) {
      if (err) {
        callback(err);
        return;
      }

      updateCache(self, result, callback);
    });
  });
};
